...
class Chatbot:
    ...
    def __init__(self, client):
        ...
        self.message = None
        # Assuming some properties are directly related to message handling
    ...
    def prepare_message_format(self, messages):
        ...
        self.logger.log(f"Formatted Messages:\n{self.messages_formatted}", 'info', 'Trinity')

    def choose_message(self):
        key_count = len(self.messages)
        if key_count > 1:
            self.result = self.agents["choose"].run(messages=self.messages_formatted)
            try:
    ...
    async def process_chosen_message(self, message):
        ...
        self.message_details["channel"] = str(self.message["channel"])
        self.message_details["author_name"] = self.parser.format_string(self.message["author"])
        self.message_details["formatted_mentions"] = self.message["formatted_mentions"]
    ...
    async def interact_with_agent(self, agent_key, message, history, user_history, additional_params=None):
        ...
        agent_params = {
            ...
            "username": get_value_or_none(self.message_details.get("author_name")),
            "new_messages": get_value_or_none(self.messages_formatted),
        ...
        #     "username": self.message_details["author_name"],
        ...
        if self.cognition['reflection'] and "Choice" in self.cognition['reflection']:
            if cognition["Choice"] == "respond":
                ...
                new_response_params = {
                    ...
                    "thought": cognition.get("Inner Thought", ""),
    ...
    def save_to_collection(self, collection_name, response_message, chat_message, metadata_extra=None):
        ...
        metadata = {
            ...
            "EmotionalResponse": self.cognition["thought"]["Emotion"] | None,
            "Inner_Thought": self.cognition["thought"]["Inner Thought"] | None,
            "User": self.message_details["author_name"],
            "Mentions": self.message_details["formatted_mentions"],
            "Channel": self.message_details["channel"]
        }
        if metadata_extra:
    ...
    def save_channel_memory(self, bot_response):
        collection_name = f"a{self.message_details['channel']}-chat_history"
    ...
    def save_user_history(self, bot_response, user_chat):
        collection_name = f"a{self.message_details['author_name']}-chat_history"
        self.save_to_collection(collection_name, bot_response, user_chat)

    def save_memory(self, bot_response):
        user_message = str(self.message['message'])
    ...
    async def chat_manager(self, message):
        chat_log = self.fetch_history(self.message_details['channel'])
        user_log = self.fetch_history(self.message_details['author_name'], query=message, is_user_specific=True)
